...
class HiddenMarkovViterbi:
    ...
    def print_delta_t(self, A, B, deltas, i, index_of_o, real_i, real_t, t):
        if self.verbose:
            print("delta%d(%d) = max[delta%d(j) * aj%d] * b%d(o%d) = %.5f * %.5f = %.10f"
    ...
    def print_delta_t1(self, B, PI, deltas, i, index_of_o, real_i, t):
        if self.verbose:
            print("delta1(%d) = pi%d * b%d(o1) = %.5f * %.5f = %.10f"
                  % (real_i, real_i, real_i, PI[0][i], B[i][index_of_o], deltas[i][t]))


Q = [1, 2]
V = ['smile', 'frown', 'laugh', 'yell']
A = [[0.8, 0.2], [0.2, 0.8]]
B = [[0.4, 0.1, 0.3, 0.2], [0.1, 0.4, 0.2, 0.3]]
O = ['smile', 'yell', 'frown', 'frown', 'laugh']
PI = [[1, 0]]
